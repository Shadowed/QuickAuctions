...
local function updateWhitelist(container)
	...
	local row = 0
	for id, name in pairs(QuickAuctions.db.realm.whitelist) do
		row = row + 1
		if( row % 2 == 0 ) then
...
deleteWhitelist = function(widget, event)
	QuickAuctions.db.realm.whitelist[widget:GetUserData("id")] = nil
...
local function addWhitelist(widget, event, value)
	...
	QuickAuctions.db.realm.whitelist[string.lower(value)] = value
