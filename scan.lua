...
function Scan:AddAuctionRecord(name, link, owner, quantity, bid, buyout)
	...
	if( not QuickAuctions.db.realm.player[owner] ) then
	...
	for _, record in pairs(auctionData[link].records) do
		if( record.owner == owner and record.buyout == buyout and record.bid == bid ) then
			...
			record.isPlayer = QuickAuctions.db.realm.player[owner]
			return
		end
	end
	
	-- Nothing available, create a new one
	table.insert(auctionData[link].records, {owner = owner, buyout = buyout, bid = bid, isPlayer = QuickAuctions.db.realm.player[owner], quantity = quantity})
end
...
function Scan:IsLowestAuction(name, buyout, bid)
	...
	for _, record in pairs(auctionData[link].records) do
		...
		if( QuickAuctions.db.realm.whitelist[record.owner] ) then
...
function Scan:GetLowestAuction(link)
	...
	for _, record in pairs(auctionData[link].records) do
		if( not record.isPlayer and record.buyout == buyout ) then
			isPlayer = nil
			if( not QuickAuctions.db.realm.whitelist[record.owner] ) then
				isWhitelist = nil
			end
